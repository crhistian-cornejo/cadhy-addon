...
class CADHY_OT_SetupRender(Operator):
    ...
    create_camera: BoolProperty(
        name="Create Camera",
        description="Create a camera positioned to view the channel",
        default=True
    )
    
    create_lights: BoolProperty(
        name="Create Lights",
        description="Create sun and fill lights",
        default=True
    )
    
    use_hdri: BoolProperty(
        name="Use HDRI",
        description="Set up HDRI environment (requires HDRI file)",
        default=False
    )
    
    camera_distance: FloatProperty(
        name="Camera Distance",
        description="Distance multiplier for camera placement",
        default=2.0,
        min=0.5,
        max=10.0
    )
