...
from ...core.io.export_mesh import export_mesh, ExportFormat, export_cfd_package
...
class CADHY_OT_ExportCFD(Operator):
    ...
    cleanup_mesh: BoolProperty(
        name="Clean Up Mesh",
        description="Apply CFD cleanup (merge doubles, triangulate, fix normals)",
        default=True
    )
    
    validate_before_export: BoolProperty(
        name="Validate Before Export",
        description="Check mesh validity before exporting",
        default=True
    )
    ...
    def execute(self, context):
        ...
        with OperationLogger("Export CFD Mesh", self) as logger:
            ...
            if success:
                ...
                self.report({'ERROR'}, f"Failed to export mesh")
                return {'CANCELLED'}
